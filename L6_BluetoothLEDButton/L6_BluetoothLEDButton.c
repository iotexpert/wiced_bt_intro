...
#include "L6_BluetoothLEDButton_db.h"
 ...
 * Function Prototypes
 ******************************************************************/
static void                   l5_bluetoothled_app_init               ( void );
static wiced_bt_dev_status_t  l5_bluetoothled_management_callback    ( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data );
static void                   l5_bluetoothled_set_advertisement_data ( void );
static void                   l5_bluetoothled_advertisement_stopped  ( void );
static void                   l5_bluetoothled_reset_device           ( void );
/* GATT Registration Callbacks */
static wiced_bt_gatt_status_t l5_bluetoothled_write_handler          ( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id );
static wiced_bt_gatt_status_t l5_bluetoothled_read_handler           ( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id );
static wiced_bt_gatt_status_t l5_bluetoothled_connect_callback       ( wiced_bt_gatt_connection_status_t *p_conn_status );
static wiced_bt_gatt_status_t l5_bluetoothled_server_callback        ( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data );
static wiced_bt_gatt_status_t l5_bluetoothled_event_handler          ( wiced_bt_gatt_evt_t  event, wiced_bt_gatt_event_data_t *p_event_data );
static uint32_t               hci_control_process_rx_cmd             ( uint8_t* p_data, uint32_t len );
#ifdef HCI_TRACE_OVER_TRANSPORT
static void                   l5_bluetoothled_trace_callback         ( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data );
#endif
...
const uint16_t l5_bluetoothled_gatt_db_ext_attr_tbl_size = ( sizeof ( l5_bluetoothled_gatt_db_ext_attr_tbl ) / sizeof ( gatt_db_lookup_table ) );
...
void application_start(void)
{
    ...
    wiced_bt_stack_init(l5_bluetoothled_management_callback, &wiced_bt_cfg_settings, wiced_bt_cfg_buf_pools);
}

/*
 * This function is executed in the BTM_ENABLED_EVT management callback.
 */
void l5_bluetoothled_app_init(void)
{
    ...
    l5_bluetoothled_set_advertisement_data();

    /* Register with stack to receive GATT callback */
    wiced_bt_gatt_register( l5_bluetoothled_event_handler );
...
/* Set Advertisement Data */
void l5_bluetoothled_set_advertisement_data( void )
{
...
void l5_bluetoothled_advertisement_stopped( void )
{
...
void l5_bluetoothled_reset_device( void )
{
...
wiced_bt_dev_status_t l5_bluetoothled_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        wiced_bt_dev_register_hci_trace(l5_bluetoothled_trace_callback);
#endif
...
#if 0
        if (l5_bluetoothled_read_link_keys( &p_event_data->paired_device_link_keys_request ))
        {
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
        ...
        {
            l5_bluetoothled_advertisement_stopped();
        }
        break;
...
/* Get a Value */
wiced_bt_gatt_status_t l5_bluetoothled_get_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t max_len, uint16_t *p_len )
{
    int i = 0;
    ...
    for (i = 0; i < l5_bluetoothled_gatt_db_ext_attr_tbl_size; i++)
    {
        if (l5_bluetoothled_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            // Detected a matching handle in external lookup table
            isHandleInTable = WICED_TRUE;
            // Detected a matching handle in the external lookup table
            if (l5_bluetoothled_gatt_db_ext_attr_tbl[i].cur_len <= max_len)
            {
                // Value fits within the supplied buffer; copy over the value
                *p_len = l5_bluetoothled_gatt_db_ext_attr_tbl[i].cur_len;
                memcpy(p_val, l5_bluetoothled_gatt_db_ext_attr_tbl[i].p_data, l5_bluetoothled_gatt_db_ext_attr_tbl[i].cur_len);
...
/* Set a Value */
wiced_bt_gatt_status_t l5_bluetoothled_set_value( uint16_t attr_handle, uint16_t conn_id, uint8_t *p_val, uint16_t len )
{
    int i = 0;
    wiced_bool_t isHandleInTable = WICED_FALSE;
    ...
    for (i = 0; i < l5_bluetoothled_gatt_db_ext_attr_tbl_size; i++)
    {
        if (l5_bluetoothled_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            validLen = (l5_bluetoothled_gatt_db_ext_attr_tbl[i].max_len >= len);
            if (validLen)
            {
                // Value fits within the supplied buffer; copy over the value
                l5_bluetoothled_gatt_db_ext_attr_tbl[i].cur_len = len;
                memcpy(l5_bluetoothled_gatt_db_ext_attr_tbl[i].p_data, p_val, len);
                res = WICED_BT_GATT_SUCCESS;
...
wiced_bt_gatt_status_t l5_bluetoothled_write_handler( wiced_bt_gatt_write_t *p_write_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Write Request */
    status = l5_bluetoothled_set_value(p_write_req->handle, conn_id, p_write_req->p_val, p_write_req->val_len);

    return status;
}

/* Handles Read Requests received from Client device */
wiced_bt_gatt_status_t l5_bluetoothled_read_handler( wiced_bt_gatt_read_t *p_read_req, uint16_t conn_id )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_INVALID_HANDLE;

    /* Attempt to perform the Read Request */
    status = l5_bluetoothled_get_value(p_read_req->handle, conn_id, p_read_req->p_val, *p_read_req->p_val_len, p_read_req->p_val_len);
...
wiced_bt_gatt_status_t l5_bluetoothled_server_callback( uint16_t conn_id, wiced_bt_gatt_request_type_t type, wiced_bt_gatt_request_data_t *p_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATTS_REQ_TYPE_READ:
        status = l5_bluetoothled_read_handler( &p_data->read_req, conn_id );
        break;
    case GATTS_REQ_TYPE_WRITE:
        status = l5_bluetoothled_write_handler( &p_data->write_req, conn_id );
        break;
    }
...
/* GATT Event Handler */
wiced_bt_gatt_status_t l5_bluetoothled_event_handler( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_event_data )
{
    wiced_bt_gatt_status_t status = WICED_BT_GATT_ERROR;
    ...
    case GATT_CONNECTION_STATUS_EVT:
        status = l5_bluetoothled_connect_callback( &p_event_data->connection_status );
        break;
    case GATT_ATTRIBUTE_REQUEST_EVT:
        p_attr_req = &p_event_data->attribute_request;
        status = l5_bluetoothled_server_callback( p_attr_req->conn_id, p_attr_req->request_type, &p_attr_req->data );
        break;
    default:
        status = WICED_BT_GATT_SUCCESS;
        break;
...
void l5_bluetoothled_trace_callback( wiced_bt_hci_trace_type_t type, uint16_t length, uint8_t* p_data )
{
